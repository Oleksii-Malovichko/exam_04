...
int	sandbox(void (*f)(void), unsigned int timeout, bool verbose)
{
	...
	if (WIFSIGNALED(status)) // проверяет был ли процесс уничтожен сигналом (sigkill, sigsegv)
	{
		int sig = WTERMSIG(status); // извлекает номер сигнала, вызвавшего завершение процесса
		if (verbose)
